...
#include "ns3/pfifo-fast-queue-disc.h" // Include the PfifoFastQueueDisc header
...
int main(int argc, char* argv[]) {
    // Create nodes
    NodeContainer nodes;
    nodes.Create(20); // Create 10 nodes

    // Install mobility model (optional)
    // Create and configure a mobility model
   ...
   // Install the mobility model on nodes
    ...
    NetDeviceContainer devices = wifi.Install(phy, mac, nodes);
     ...
     // Create and configure TrafficControlHelper
    TrafficControlHelper tcHelper;
    tcHelper.SetQueueLimits("ns3::PfifoFastQueueDisc", "MaxSize", StringValue("100p"));

    // Attach the queue discipline to all devices
    tcHelper.Install(devices);
    ...
    UdpEchoClientHelper echoClient(interfaces.GetAddress(0), 49);
